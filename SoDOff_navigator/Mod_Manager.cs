...
namespace SoDOff_navigator
{
    public partial class Mod_Manager : Form
    {
        ...
        private void btn_update_mods_Click(object sender, EventArgs e)
        {
            // Löschen der alten Dateien, falls vorhanden
            if (File.Exists("modslist.txt"))
            {
                File.Delete("modslist.txt");
            }
            if (File.Exists("outdated.txt"))
            {
                File.Delete("outdated.txt");
            }
            Thread.Sleep(5000);
            // Herunterladen der neuen Dateien
            Main.main.WriteLog = "[Mod Manager] downloading mods list from remote server..." + "\n";
            WebClient Client = new WebClient();
            Client.DownloadFile("https://raw.githubusercontent.com/tobyw121/SoDOff_navigator_modslist/main/modslist.txt", "modslist.txt");
            ...
            // Neu laden des Fensters
            ReloadModManagerWindow();
            ParseModAuthors();
            UpdateModsList();
            InitializeComponent();
        }

        private void ReloadModManagerWindow()
        {
            // Erstellen einer neuen Instanz des Mod_Manager-Formulars
            Mod_Manager modManagerForm = new Mod_Manager();


            // Schließen des aktuellen Mod_Manager-Formulars
            this.Close();

            // Anzeigen des neuen Mod_Manager-Formulars
            modManagerForm.Show();
        }
    }
}
